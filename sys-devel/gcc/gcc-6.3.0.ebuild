...
IUSE="$IUSE cxx nls vanilla doc multilib altivec libssp ssp pie pch hardened graphite sanitize" # other stuff
...
# Graphite support:
SRC_URI="$SRC_URI graphite? ( http://www.bastoul.net/cloog/pages/download/count.php3?url=./cloog-${CLOOG_VER}.tar.gz http://isl.gforge.inria.fr/isl-${ISL_VER}.tar.bz2 )"
...
src_configure() {
	...
	if is_crosscompile; then
		...
		confgcc+=" --enable-shared --enable-threads=posix $(use_enable openmp libgomp)"
	fi
